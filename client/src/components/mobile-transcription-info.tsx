...
import { isMobileDevice, canUseLocalTranscription } from '@/lib/device-detection'
...
export function MobileTranscriptionInfo({
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1 pr-6">
            ...
            <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
              Your device is using server transcription for best performance. Audio is processed
              securely and <strong>deleted immediately</strong> after transcription - nothing is stored.
            </p>
  ...
  return (
    <div className="relative p-4 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg">
      ...
      <div className="flex gap-3">
        <HardDrive className="text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" size={20} />
        <div className="flex-1 pr-6">
          ...
          <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
            {canUseLocal ? (
              <>
                Running AI locally on your device. This uses more memory and may be slower.
                Switch to <strong>server mode</strong> for better performance.
              </>
            ) : (
              <>
                Local transcription may not work well on your device due to memory constraints.
                Consider using <strong>server mode</strong> instead.
              </>
            )}
          </p>
        </div>
...
export function MobileServerModeIndicator({ remaining }: { remaining: number }) {
  const isMobile = isMobileDevice()
  ...
  return (
    <div className="inline-flex items-center gap-1.5 px-2 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded text-xs">
      ...
      <span className="text-blue-500 dark:text-blue-400">({remaining} left)</span>
    </div>
  )
}
